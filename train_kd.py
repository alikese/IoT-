...
from net import TeacherMLP,StudentMLP
from get_dataset import get_data,NpyFlowDataset

parser = argparse.ArgumentParser(description='train kd')

# various path
parser.add_argument('--save_root', type=str, default='./results/lwm2017_binary', help='models and logs are saved here')
parser.add_argument('--img_root', type=str, default='./CIC17_18/CIC17_18', help='path name of image dataset')
parser.add_argument('--s_init', type=str, default='./results/base2017_binary/try/initial_rrMLP.pth.tar', help='initial parameters of student model')
parser.add_argument('--t_model', type=str, default='./results/base2017_binary/try/model_best.pth.tar', help='path name of teacher model')

# training hyper parameters
parser.add_argument('--print_freq', type=int, default=300, help='frequency of showing training results on console')
parser.add_argument('--epochs', type=int, default=80, help='number of total epochs to run')
...
# parser.add_argument('--attack_max_samples', type=int, default=10000,
...
# hyperparameter
parser.add_argument('--kd_mode', type=str, default='lwm', help='mode of kd, which can be:'
...
parser.add_argument('--w_dist', type=float, default=1.0, help='weight for RKD distance')
parser.add_argument('--w_angle', type=float, default=1.0, help='weight for RKD angle')
...
#	logging.info("unparsed_args = %s", unparsed)
	...
	X_train, X_test, y_train, y_test, feature_min, feature_max, unknown_data_list, unknown_label_list, num_classes, feature_num, attack_classes = get_data(
		args)
	snet = StudentMLP(feature_num,num_classes)
	...
	tnet = TeacherMLP(feature_num,num_classes)
	...
	for epoch in range(1, args.epochs+1):
		...
		test_top1,f1 = test(test_loader, nets, criterions, epoch)
...
def train(train_loader, nets, optimizer, criterions, epoch):
	...
	for i, (img, target) in enumerate(train_loader, start=1):
		...
		if args.kd_mode in ['logits', 'st']:
			...
			kd_loss = (criterionKD(rb1_s, rb1_t.detach()) +
					   criterionKD(rb2_s, rb2_t.detach()) +
					   criterionKD(rb3_s, rb3_t.detach())) / 3.0 * args.lambda_kd
			...
			kd_loss = criterionKD(out_s, rb2_s, out_t, rb2_t, target) * args.lambda_kd
		...
		if i % args.print_freq == 0:
			...
			log_str = ('Epoch[{0}]:[{1:03}/{2:03}] '
					   ...
					   epoch, i, len(train_loader), batch_time=batch_time, data_time=data_time,
					   cls_losses=cls_losses, kd_losses=kd_losses, top1=top1,f1=f1_epoch))
			logging.info(log_str)
...
def test(test_loader, nets, criterions, epoch):
	...
	f_l = [cls_losses.avg, kd_losses.avg, top1.avg,f1]
	logging.info('Cls: {:.4f}, KD: {:.4f}, Prec@1: {:.2f}, F1-macro: {:.4f}'.format(*f_l))
